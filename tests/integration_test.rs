    ...
    match &result {
        Ok(_) => {
            ...
            let target_client = database_replicator::postgres::connect(&target_url)
                ...
                .unwrap();
            let _ =
                database_replicator::replication::drop_subscription(&target_client, sub_name).await;

            let source_client = database_replicator::postgres::connect(&source_url)
                .await
                .unwrap();
            let _ =
                database_replicator::replication::drop_publication(&source_client, pub_name).await;
        }
        Err(e) => {
