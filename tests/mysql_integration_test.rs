...
async fn create_test_mysql_tables(mysql_url: &str) -> anyhow::Result<()> {
    ...
    let mut conn = database_replicator::mysql::connect_mysql(mysql_url).await?;
...
async fn test_mysql_full_replication_integration() {
    ...
    let result = commands::init(
        ...
        &target_url,
        true,
        database_replicator::filters::ReplicationFilter::empty(),
        false,
        false,
        true,
        false,
...
async fn test_mysql_all_data_types() {
    ...
    let mut conn = database_replicator::mysql::connect_mysql(&mysql_url)
        .await
